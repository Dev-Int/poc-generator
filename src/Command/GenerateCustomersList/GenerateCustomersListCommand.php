...
#[AsCommand(
    name: 'generate:customers-list',
    description: 'Generate list of customers in json file.',
)]
final class GenerateCustomersListCommand extends Command
{
    private const EXPORT_DIRECTORY = 'var/storage/';
    private const FILE_NAME = 'customers.xls';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            $pb = new ProgressBar($io, \count($customersList));
            $pb->setFormat(' %current%/%max% customers [%bar%] %percent:3s%% %elapsed:6s%/%estimated:-6s% %memory:6s%');
            $pb->start();

            foreach ($customersList as $idx => $customerAddress) {
                $this->prepareRow($customerAddress, $idx + 1, $activeWorksheet);
            ...
            $event = $stopwatch->stop('customers');
    ...
    private function getCustomersList(): array
    {
        ...
        foreach ($customers as $customer) {
            foreach ($customer->getAddresses() as $address) {
                $customersList[] = $customer->getByAddressName($address->getName());
            }
        }
